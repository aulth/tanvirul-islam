...
const page = () => {
  ...
  const alumniData = users.filter(user => {
    const fromYear = parseInt(user.from);
    const toYear = parseInt(user.to);

    // Filter condition: Check if the user's 'to' year is less than or equal to the current year
    // and the 'from' year is less than or equal to the current year
    return toYear <= currentYear && fromYear <= currentYear;
  });
  // const alumniData = [
  //     {
  //       name: "John Smith",
